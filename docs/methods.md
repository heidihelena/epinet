...
comparator), `xgboost` (optional dependency), or `mlp` (a tiny optional PyTorch
neural comparator). Tuning is **imbalance-aware** where the estimator exposes
the relevant controls: the random-forest and logistic grids include class
weighting, the MLP uses balanced loss weighting, and every grid selects on
...
`--model xgboost` or `--model mlp` fails closed rather than tuning on leaked
...
## Graph Semantics Gate

EpiNet now records two pieces of graph metadata in `analysis.yaml`:

- `analysis.graph.semantics`: `similarity`, `observed_relation`, or
  `unspecified`.
- `analysis.graph.edge_timing`: `pre_outcome`, `post_outcome`, `mixed`, or
  `unknown`.

These fields do not change graph construction. They constrain what the report
allows the user to claim. A similarity graph is a transformation of the feature
table, so it can support clustering, contestability, and feature-space
exploration, but it does not by itself establish contact, referral, exposure, or
message-passing effects. An observed relation graph can support graph-aware
predictive claims only when the edge was knowable before the outcome; otherwise
post-outcome or mixed-time leakage remains unresolved.

The scientific claims check therefore includes a **Graph semantics** gate in
`claims_check.json`, the model card, the HTML report, and the Workbench UI. This
is the practical boundary between ordinary tabular machine learning on derived
features and a defensible network-epidemiology claim.
