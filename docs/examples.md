...
```bash
python -m vahtian.epinet.toolkit --outcome-column Outcome --target-outcome 1
```

Use explicit target nodes:

```bash
python -m vahtian.epinet.toolkit --target-nodes Node_1,Node_5 --no-run-model
```

Limit the sources:

```bash
python -m vahtian.epinet.toolkit --source-nodes Node_0,Node_9 --target-nodes Node_42 --no-run-model
```

Treat edges as directed:

```bash
python -m vahtian.epinet.toolkit --directed --target-nodes Node_42 --no-run-model
```

Use an edge weight column as path distance:

```bash
python -m vahtian.epinet.toolkit --weight-column Weight --path-mode distance --target-nodes Node_42 --no-run-model
```
...
```bash
python -m vahtian.epinet.toolkit --weight-column Weight --path-mode strength --target-nodes Node_42 --no-run-model
```
...
```bash
python -m vahtian.epinet.toolkit --run-clusters --distance-metric mahalanobis --n-clusters 0
```
